  ...
  <div class="upload-box">
    <el-upload
      ...
      <template v-if="file?.filePath">
        <img :src="getFileUrl(file?.filePath)" class="upload-image" />
        <div class="upload-handle" @click.stop>
          <div v-if="!self_disabled" class="handle-icon" @click="editImg(file)">
    ...
    <el-image-viewer
      v-if="imgViewVisible"
      :url-list="[getFileUrl(file?.filePath)]"
...
interface UploadFileProps {
  file?: SystemAttachment.ResSystemAttachmentItem; // 文件 ==> 必传
...
interface UploadEmits {
  ...
  (e: "update:attachmentId", value: number): void;
}
...
const handleHttpUpload = async (options: UploadRequestOptions) => {
  ...
  try {
    if (props.attachmentId !== 0 && props.attachmentId !== undefined && props.attachmentId !== null) {
      const { data } = await putSystemAttachmentApi(Number(props.attachmentId), formData);
 ...
 * @description 编辑文件
 * */
const editImg = (handel: SystemAttachment.ResSystemAttachmentItem) => {
  emit("update:file", handel);
  emit("update:attachmentId", handel.id);
