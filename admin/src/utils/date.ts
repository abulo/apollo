...
export function defRangeShortcuts() {
  ...
  return {
    ...
    本周: () => {
      return [startOfWeek(nowDate, { weekStartsOn: 1 }).getTime(), endOfWeek(nowDate, { weekStartsOn: 1 }).getTime()] as const;
    },
    本月: () => {
      return [startOfMonth(nowDate).getTime(), endOfMonth(nowDate).getTime()] as const;
    },
    上月: () => {
      return [startOfMonth(subMonths(nowDate, 1)).getTime(), endOfMonth(subMonths(nowDate, 1)).getTime()] as const;
    }
  };
}
